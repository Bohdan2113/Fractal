...
function SetBoundaries(form, type) {
  ...
  if (type === "Algebraical") {
    ...
    iterationsField.max = 1000;
    iterationsField.value = 100;
  } else if (type === "Minkovskogo") {
    iterationsField.min = 0;
    iterationsField.max = 6;
    iterationsField.value = 4;
  }
}
...
function DrawCoords(canvas, unitCount) {
  ...
  DrawLine(
    canvas,
    { x: leftSizingLuft, y: 0 },
    { x: leftSizingLuft, y: canvas.height - bottomSizingLuft }
  ); // Verrtical line
  DrawLine(
    canvas,
    { x: leftSizingLuft, y: canvas.height - bottomSizingLuft },
    { x: canvas.width, y: canvas.height - bottomSizingLuft }
  ); // Horizontal line
...
function DrawLine(canvas, pStart, pEnd, width = 1, color = "#000000") {
  const ctx = canvas.getContext("2d");
